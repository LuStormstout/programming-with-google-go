...
// 在大部分情况下，值类型的变量的值存储在栈中，引用类型的变量的值存储在堆中，或者存储在栈中，但是值本身是指针
// 值类型的变量的值在内存中的地址不会改变，但指针指向的值是可以改变的
// 引用类型的变量的值在内存中的地址会改变，但指针指向的值是不可以改变的
...
/*
	...
	总体来说，你的描述是准确的，上述修改仅是为了更清晰地表达各个方面。
*/
/*
